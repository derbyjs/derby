...
If a bound template tag is not fully contained by an HTML tag, Derby will wrap the template in an `<ins>` element. Most browsers will display `<ins>` elements with an underline by default, which can be removed by adding `ins{text-decoration:none}` to the page's stylesheet.
