...
app.use '/chat', gzip.staticGzip(__dirname + '/public', MAX_AGE_ONE_YEAR)
app.use express.favicon()
app.use express.cookieParser()
app.use express.session secret: 'dont_tell'
app.use gzip.gzip()

chat = require './chat.server'
store = chat.createStore listen: app, redis: {db: 3}
store.flush()
newUserId = 0
...
app.get '/chat/:room?', (req, res) ->
  ...
  return res.redirect '/chat/lobby' unless room && /^[-\w ]+$/.test room
  _room = room.toLowerCase().replace /[_ ]/g, '-'
  return res.redirect "/chat/#{_room}" if _room != room
...
app.listen 3003
console.log "Go to: http://localhost:3003/chat/lobby"
