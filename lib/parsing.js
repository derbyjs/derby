...
templates.View.prototype._parse = function() {
  var template = createTemplate(this.source, this);
...
function parseBlockExpression(expression) {
  ...
  if (expression.meta.isEnd) {
    ...
    if (!(
      ...
      throw new Error('Mismatched ending template tag: ' + expression.meta.source);
    }
...
function parseViewElement(element) {
  ...
  if (!viewAttributes.content && remaining.length) {
    viewAttributes.content = new saddle.Template(remaining);
  }
...
function parseContentAttributes(content, view, viewAttributes) {
  ...
  for (var i = 0, len = content.length; i < len; i++) {
    var item = content[i];
    var tagName = (item instanceof saddle.Element) && item.tag;

    if (view.attributesMap && view.attributesMap[tagName]) {
      viewAttributes[tagName] = new saddle.Template(item.content);

    } else if (view.sectionsMap && view.sectionsMap[tagName]) {
      var attributes = viewAttributesFromElement(item);
      var sectionItemArgs = [];
      for (var key in attributes) {
        var keyExpression = new saddle.LiteralExpression(key);
        var expression = attributes[key];
        sectionItemArgs.push(keyExpression, expression);
      }
      if (item.content.length) {
        var keyExpression = new saddle.LiteralExpression('content');
        var expression = new saddle.Template(element.content);
        sectionItemArgs.push(keyExpression, expression);
      }
      var sectionItem = new expressions.FnExpression('{}', sectionItemArgs);
      var section = viewAttributes[tagName] || new expressions.FnExpression('[]', []);
      section.args.push(sectionItem);
