...
App.prototype._watchViews = function(filenames, filename, namespace) {
  var app = this;
  var watcher = chokidar.watch(filenames, {ignoreInitial: true});
...
App.prototype._updateScriptViews = function() {
  this.script = this.script.replace(
    /\/\*DERBY_SERIALIZED_VIEWS\*\/.*\/\*DERBY_SERIALIZED_VIEWS_END\*\//
  , '/*DERBY_SERIALIZED_VIEWS*/' + this._viewsSource() + '/*DERBY_SERIALIZED_VIEWS_END*/'
  );
};
