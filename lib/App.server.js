...
App.prototype.scripts = function(store, options) {
  ...
  if (!this.script) {
    var pending = [];
    this.setScripts(store, options, function(err) {
      if (err) throw err;
      if (!pending) return;
...
App.prototype.setScripts = function(store, options, cb) {
  ...
  setImmediate(function() {
    app.bundle(store, options, finish);
  });
  function finish(err, source, map) {
...
App.prototype.bundle = function(store, options, cb) {
  ...
  var viewsSource = 'module.exports = ' + this.views.serialize() + ';\n';
  ...
  return store.bundle(this.filename, options, cb);
};
...
App.prototype.loadStyles = function(filename, options) {
  ...
  this.views.register('app:Styles', '<style id="_css">' + css + '</style>');
};
