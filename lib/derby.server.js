...
function createApp(appModule) {
  ...
  view._scriptMapPath = view._scriptPath + '.map';
...
function scripts(store) {
  ...
  // Load templates and bundle on server startup to be ready to serve requests
  process.nextTick(function() {
    if (process.env.DERBY_PRELOAD === 'none') return;
    view._load(false, function(err) {
  ...
  function scriptsMiddleware(req, res, next) {
    if (req.url === view._scriptPath) {
      view._loadBundle(function(err, bundle, map) {
        if (err) return next(err);
        res.type('js');
        res.send(bundle);
      });
    } else if (req.url === view._scriptMapPath) {
      view._loadBundle(function(err, bundle, map) {
        if (err) return next(err);
        res.type('json');
        res.send(map);
