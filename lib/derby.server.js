...
function run(filename, port, cb) {
  ...
  if (isProduction) return require(filename).listen(port, listenCallback);
  derbyCluster.run(filename, port, listenCallback);
}
