...
var derby = module.exports = util.mergeAll(Object.create(racer), {
  // TODO: Remove in lieu with get / set methods
  options: {}
...
, createStatic: function(root) {
    return new Static(root, derby._libraries);
  }

, createLibrary: function(libraryModule, options) {
  if (!options) options = {};
  var libraryExports = libraryModule.exports
    , filename = libraryModule.filename
    , root = path.dirname(filename)
    , name = options.name || path.basename(root)
    , view = new View

  derby._libraries[name] = {
    view: view
  };

  // Load library files and make templates
  files.library(root, function(err, components) {
    var templates = {}
      , instances = {}
    ...
    for (componentName in components) {
      ...
      merge(templates, component.templates);
      merge(instances, component.instances);
    }
