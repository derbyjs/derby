...
EventModel.prototype._addItemContext = function(context) {
  ...
  var itemContexts = this.itemContexts || (this.itemContexts = new ItemContextsMap());
  context._eventModels || (context._eventModels = new EventModelsMap());
  itemContexts[context._id] = context;
  context._eventModels[this.id] = this;
};
...
EventModel.prototype.removeItemContext = function(context) {
  if (!context._eventModels) return;
  for (var id in context._eventModels) {
    var eventModel = context._eventModels[id];
    if (eventModel.itemContexts) {
      delete eventModel.itemContexts[context._id];
    }
  }
  context._eventModels = null;
};
