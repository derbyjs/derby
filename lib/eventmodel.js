var lookup = require('derby-expressions').expressions.lookup;
...
ModelRef.prototype.update = function() {
  var newItem = lookup(expandSegments(this.segments), this.model.data);
...
EventModel.prototype.arrayLookup = function(model, segmentsBefore, segmentsInside) {
  var item = lookup(expandSegments(segmentsInside), model.data);
...
EventModel.prototype._addBinding = function(binding) {
  ...
  if (!this.bindings) this.bindings = new BindingsMap();
...
// This is used for objects (contexts in derby's case) that have a .item property which refers to an array index.
...
EventModel.prototype.mutate = function(segments, fn) {
  ...
  for (var i = 0, len = segments.length; i++ < len;) {
    var starSegments = segments.slice(0, i);
    starSegments.push('*');
    this._each(starSegments, 0, childSet);
  }
};
