...
function EventModel(meta) {
  ...
  // The bindings to this event model object. Null or an object from id -> Dependancy.
  ...
  // Unbound dependancies are dependancies which don't exist in the actual
  // event tree, and are only updated via their parent dependancies. This will
  // only be set on the root EventModel object.
  //
  // This comes up for function expressions - eg, {{ fn(x) }} has a dependancy
  // on x but has no segment list itself.
  this.unboundDependancies = null;
}
...
EventModel.prototype.isEmpty = function() {
  ...
  if (this.arrayByValue) {
    for (var i = 0; i < this.arrayByValue.length; i++) {
      if (this.arrayByValue[i] != null) return false;
    }
...
EventModel.prototype._removeBinding = function(binding) {
  if (this.bindings) {
    var idx = this.bindings.indexOf(context);
...
EventModel.prototype.addBinding = function(binding) {
  ...
  if (binding.isItem) {
    //console.log('added item binding', context.item, expression.resolve(context));

    var container = this.at(expression.resolve(context));
    container._addItemContext(context);
  } else {
    //console.log('added binding', expression.resolve(context));

    var dependencies = expression.dependencies(context);

    for (var i = 0; i < dependencies.length; i++) {
      // TODO: star bindings have a * as the last dependancy.
      var container = this.at(dependencies[i]);
...
// on this node. See recursiveUpdate() below if you want to update entire
// subtrees.
