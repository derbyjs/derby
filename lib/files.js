...
// Find path for a file named `root/name` and having `extension`. When done,
// call a `callback` with a parameter set to found `path` or with `null`.
//
...
// ## Loading templates from template file (also loading import template files)
//
 ...
 * @param  {[type]} currentNs [description]
 * @return {[type]}           [description]
 */
...
/**
 * Parse template file given as a string `file` uting `parse` function exported
 * from `html-util` package.
 *
 * @param  {[type]} root       [description]
 ...
 * @param  {[type]} alias      [description]
 * @param  {[type]} currentNs  [description]
 * @param  {[type]} matchesGet [description]
