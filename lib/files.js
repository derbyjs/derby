...
function parseName(parentFilename, options) {
  ...
  , require: '/' + unixRelative(process.cwd(), parentFilename)
  };
}
...
function watch(dir, type, onChange) {
  ...
  files(dir, extension).forEach(function(path) {
    fs.readFile(path, 'utf8', function(err, file) {
      try {
        watchFn(path, hashFile(file));
      } catch (e) {
        console.error('File watch failed for: ' + path)
      }
    });
  });

  function checkModified(path, hash, callback) {
    fs.readFile(path, 'utf8', function(err, file) {
      var newHash = hashFile(file);
      if (hash === newHash) return;
      onChange(path);
      callback(newHash);
    })
  }
  function systemWatch(path, hash) {
    fs.watch(path, function() {
      checkModified(path, hash, function(newHash) {
        hash = newHash;
      });
    });
  }
  function pollWatch(path, hash) {
