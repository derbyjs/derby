...
function library(root, callback) {
  ...
  function libraryFile(file, callback) {
    ...
    fs.stat(path, function(err, stats) {
      if (err) return callback(err);
      if (stats.isDirectory()) return componentDirectory(file, callback);
      callback();
    }); 
  }

  function componentDirectory(file, callback) {
    var path = root + '/' + file
      , script = require(path)
    loadTemplates(root, file, function(err, templates, instances) {
      components[file] = {
        script: script
      , templates: templates
      , instances: instances
