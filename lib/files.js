...
function css(root, clientName, compress, callback) {
  ...
  var styles = ['less', 'stylus', 'sass'];
  if (!Array.isArray(styles)) styles = [styles];

  async.map(styles, function(style, cb) {
    var compiler = styleCompilers[style];
    if (!compiler) return cb(new Error('Unable to find compiler for: ' + style));
...
function sassCompiler(root, clientName, compress, callback) {
  findPath(root, clientName, '.scss', function(path) {
    if (!path) return callback('');
    sass.render({
      file: path
    , success: function successCallback(css) {
        callback(null, css);
      }
    , error: callback
