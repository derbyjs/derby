const { util } = require('racer');

// Avoid Browserifying these dependencies
let crypto, fs, path, through;
if (module.require) {
...
module.exports = function derbyBundler(app, options) {
  ...
  console.log('use derby-bundler');
  const { App, Page } = app.derby;
  ...
  App.prototype.bundle = function(backend, options, cb) {
    ...
    backend.once('bundle', function(bundle) {
      bundle.require(path.dirname(path.resolve(__dirname, '..')), {expose: 'derby'});
      ...
      console.log('viewsFilename', viewsFilename);
  ...
  app.on('htmlDone', page => {
    const bundleScriptTag = `<script async data-derby-app src="${page.app.scriptUrl}"`;
