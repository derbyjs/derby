...
function replaceText(el, newText, transformCursor) {
  ...
  if (document.activeElement === el) {
    el.selectionStart = transformCursor(start);
    el.selectionEnd = transformCursor(end);
  }
}

function onTextInput(model, path, value) {
  var previous = model.get(path) || '';
  if (value != previous) applyChange(model, path, previous, value);
}

function applyChange(model, path, previous, value) {
