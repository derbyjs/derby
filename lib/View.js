...
function reduceStack(stack) {
  ...
  for (j = 0, len = stack.length; j < len; j++) {
    ...
    switch (item[0]) {
      ...
      case 'marker':
        html[i] += '<!--' + item[1];
        i = pushValue(html, i, item[2].id, false, true);
        html[i] += '-->';
    }
  }
  return html;
...
function Component(type, model, init, create) {
  this.type = type;
  this.model = model;
  this.init = init;
  this.create = create;
...
function createComponent(view, model, ns, name, scope, ctx, macroCtx, boundMacro) {
  ...
  var init = script.init
    ...
    , prefix = scope + '.'
    , type = ns + ':' + name
    , component = new Component(type, scoped, init, create)
  ...
  for (i = parentFnCtx.length; fnCtx = parentFnCtx[--i];) {
    ...
    fnCtx.emit('init:' + type, component);
    if (instanceName) {
  ...
  setTimeout(function() {
    ...
    for (i = parentFnCtx.length; fnCtx = parentFnCtx[--i];) {
      ...
      fnCtx.emit('create:' + type, component);
      if (instanceName) {
...
function partialFn(view, name, type, alias, render, macroCtx, macro, boundMacro) {
  ...
  if (type === 'partial') {
    return function(ctx, model, triggerPath, triggerId, value, index, listener) {
      ...
      if (alias) model.__fnCtx = model.__fnCtx.slice(0, -1);
      return out;
    }
  }
