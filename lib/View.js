...
, make: function(name, template, options, templatePath, macroAttrs) {
    var view = this
...
, _find: function(name, ns, macroAttrs) {
  ...
  if (type === 'partial') {
    return function(ctx, model, triggerPath, triggerId, value, index, listener) {
      ...
      if (alias) {
        ...
        hasScope = createComponent(view, model, name[0], name[1], scope, renderCtx, macroCtx, macroAttrs);
      } else {
...
function boundMacroName(macroAttrs, name) {
  var macroVar = name.split('.')[0]
    , attrs = lastItem(macroAttrs)
    , attr = attrs && attrs[macroVar];
  return attr && attr.$bound && attr.$matchName;
}

function boundName(macroAttrs, match, name) {
  if (!(name && match.macro)) return match.bound && name;
  if (~name.indexOf('(')) {
    var args = pathFnArgs(name)
      , i, len;
    for (i = 0, len = args.length; i < len; i++) {
      if (boundMacroName(macroAttrs, args[i])) return name;
    }
    return false;
  }
  return boundMacroName(macroAttrs, name);
}
...
function pushVar(view, ns, stack, events, macroAttrs, remainder, match, fn) {
  ...
  if (parseName = boundName(macroAttrs, match, name)) {
    last = lastItem(stack);
    wrap = match.pre ||
      !last ||
      (last[0] !== 'start') ||
...
function pushVarString(view, ns, stack, events, macroAttrs, remainder, match, fn) {
  ...
  if (boundName(macroAttrs, match, name)) {
...
function onBlock(start, end, block, queues, callbacks) {
  ...
  if (start) {
    ...
    , block: block
    , sections: []
    , macroAttrs: queue.macroAttrs.slice()
    };
    queues.push(queue);
...
function parsePartialAttr(view, viewName, events, macroAttrs, attrs, attr) {
  var value = attrs[attr]
    , match = (value && value.$match) || extractPlaceholder(value)
  if (attr === 'content') {
  ...
  if (match) {
    ...
    if (match.macro) {
      attrs[attr] = lastItem(macroAttrs)[attr];
      return parsePartialAttr(view, viewName, events, macroAttrs, attrs, attr);
    }
...
function parse(view, viewName, template, isString, onBind, macroAttrs) {
  ...
  function parseStart(tag, tagName, attrs) {
    var attr, block, out, parser, _macroAttrs
    ...
    if (isPartial(view, tagName)) {
      ...
      for (attr in attrs) {
        parsePartialAttr(view, viewName, events, macroAttrs, attrs, attr);
      }

      macroAttrs.push(attrs);
      _macroAttrs = macroAttrs
      if (isVoidComponent(view, tagName, ns)) {
        onBlock(false, true, null, queues, {
          onStart: onStart
        , onEnd: function(queues) {
            push(view, ns, stack, events, _macroAttrs, '', block);
          }
        })
      }
      return;
