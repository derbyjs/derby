  ...
  if (type === 'partial') {
    return function(ctx, model, triggerPath, triggerId, value, index, listener) {
      ...
      var renderMacroCtx = {}
        ...
        , key, val, matchName
      if (macroCtx.inherit) {
        mergedMacroCtx = merge({}, parentMacroCtx);
  ...
  function parseEnd(tag, tagName) {
    ...
    if (sectionName || endsPartial) {
      ...
      , onEnd: function(queues) {
          ...
          // Put the remaining content not in a section on the macroCtx, unless
          // "inherit" is specified, so that the parent content passes through
          if (!block.macroCtx.inherit) block.macroCtx.content = fn;
          push(view, ns, stack, events, '', block);
        }
      })
      return;
