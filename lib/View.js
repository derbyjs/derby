  ...
  if (type === 'partial') {
    return function(ctx, model, triggerPath, triggerId, value, index, listener) {
      var renderMacroCtx = {}
        ...
        , keys, i, renderCtx, key, val, matchName, scope, out;

      if (inherit) {
        macroCtx = merge({}, macroCtx);
        if (inherit === "*") {
          merge(macroCtx, parentMacroCtx);
        } else {
          keys = inherit.split(' ');
          for (i = keys.length; i--;) {
            key = keys[i];
            macroCtx[key] = parentMacroCtx[key];
          }
        }
        delete macroCtx.inherit;
      }
      for (key in macroCtx) {
        val = macroCtx[key];
