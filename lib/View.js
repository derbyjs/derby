...
function bindPathEvent(events, name, getName, partial, params, saveBlockPath) {
  events.push(function(ctx, modelEvents, dom, pathMap, view, blockPaths, triggerId) {
    var getPath = ctxPath(view, ctx, getName);
    ...
    for (var i = segments.length; i; i--) {
      var bindName = segments.slice(0, i).join('.');
      var pathId = pathMap.id(bindName);
      var listener = modelListener(params, triggerId, blockPaths, first && pathId, partial, ctx);
      if (!first || name !== getName) {
        listener.getValue = getValue;
      }
...
function bindEachPathEvent(events, name, getName, partial, params) {
  ...
  bindPathEvent(events, name, getName, partial, params);
}
...
function createComponent(view, model, Component, scope, ctx, macroCtx) {
  ...
  process.nextTick(function() {
    // If for some reason, `scoped~ does not have any data, do nothing.
    // Not sure why it would get to this state, but it does, so catch this
    // issue-inducing state here
