...
function View(libraries, app, appFilename) {
  this._libraries = libraries = (libraries && libraries.slice()) || [];
  this._libraries.map = {};
  for (var i = libraries.length; i--;) {
    var library = libraries[i] = Object.create(libraries[i]);
    library.view = Object.create(library.view);
    library.view.parent = app;
    this._libraries.map[library.ns] = library;
  }
  this.app = app || {};
...
function createComponent(view, model, Component, scope, ctx, macroCtx) {
  ...
  for (key in macroCtx) {
    ...
    if (value && value.$matchName) {
      if (value.$matchName.charAt(0) === '^') {
        var newName = val.$matchName.substring(1);
        value = Object.create(value);
        value.$matchName = dataValue(view, ctx, model, newName);
      }
      if (value && value.$matchName) {
...
function partialFn(view, name, type, alias, render, ns, macroCtx) {
  function partialBlock (ctx, model, triggerPath, triggerId, value, index, listener) {
    ...
    if (macroCtx.inherit) {
      mergedMacroCtx = {};
      derby.util.merge(mergedMacroCtx, parentMacroCtx);
      derby.util.merge(mergedMacroCtx, macroCtx);
      delete mergedMacroCtx.inherit;
    }
    for (key in mergedMacroCtx) {
      val = mergedMacroCtx[key];
      if (val && val.$matchName) {
        if (val.$matchName.charAt(0) === '^') {
          var newName = val.$matchName.substring(1);
          val = Object.create(val);
          val.$matchName = dataValue(view, ctx, model, newName);
        }
        if (val && val.$matchName) {
...
function parseAttr(view, viewName, events, tagName, attrs, attr) {
  ...
  var match = extractPlaceholder(value);
  if (match && match.name && match.name.charAt(0) == '^') {
    var reference = match.name.substring(1);
    var _attrs = attrs;
    var _attr = attr;
    var _events = events;
    attrs[attr] = function (ctx, model) {
      var name = dataValue(view, ctx, model, reference);
      var match = extractPlaceholder('{' + name + '}');
      var attrOut = parseMarkup('attr', _attr, tagName, events, _attrs, name) || {}
      _attrs[_attr] = function (ctx, model) {
        return dataValue(view, ctx, model, name, escapeAttribute, true);
      }
      var boundOut = parseMarkup('bound', _attr, tagName, _events, _attrs, match) || {};
      var method = boundOut.method || 'attr';
      var property = boundOut.property || _attr;
      bindEventsByIdString(_events, reference, null, _attrs, method, property);
      return _attrs[_attr](ctx, model);
    }
    return;
  }
