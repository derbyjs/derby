// `View` is the main module for working with view templates.
//
// This module parses HTML templates using the `html-util` module's parsing
// functionality. After that all text sections from parsed HTML are processed by
// different `parse*` functions defined in this module for extracting Derby's
...
// ## Clearing state of a `this` view object
//
// Exposed as a `view.clear` method.
function clear() {
...
// Return a component constructor if it exists for a component defined by `name`
// and a `ns` namespace.
...
// ## Creating view templates programmatically
//
// See [*Creating templates* section](http://derbyjs.com/#creating_templates)
// of a Derby documentation for an API description.
//
// Make HTML from view template
//
// Exposed as a `view.make` method. Internally it is used by a `makeAll`, `find`
// and `make` itself recursivelly.
//
// Populates `this._views`.
// Cache any templates that are made programmatically so that they can be
// re-parsed with different items bound when using macros.
//
// See `find` function for usage details.
/**
 * Create view template programmatically
 *
 * @param  {String} name         Name of a template, e.g. `Body` or `customTpl`
 * @param  {String} template     HTML content of a template to be created
 * @param  {[type]} options      [description]
 * @param  {[type]} templatePath [description]
 * @param  {[type]} macroAttrs   [description]
...
// ## Make all templates one by one
//
// Exposed as a `view._makeAll` method. Used by `View.server` and `refresh`
// modules. Internally it is called only by `makeComponents` function but it is
// called on a component library's own `View` instance.
//
// This method does useful work only when `instances` are provided.
//
// Each `template` is a String containing HTML content of a template.
//
// Each `instance` is a pair represented as an Array, where first item is a
// `templatePath` String and the second one is an `options` Object.

/**
 * Make all templates one by one
 *
 * @param  {Object} templates Map where `templatePath` is used as a key
 * @param  {Object} instances Map where template names are used as a keys
 * @return {undefined}
 */
...
// Expossed as a `view._makeComponents`, not used in this module.
...
function find(name, ns, optional) {
  ...
  if (optional) return view || empty;
  return view || notFound(name, ns);
}

// ## Get a component `name` from namespace `ns`
//
...
// ## Method exposed as a `view._beforeRender`
//
...
//
// Note: `ctx` is only used to render initial `ns` and is not visible by other
...
// Create component instance using a `Component` constructor
//
// Component instance will have access to scoped model at path defined at
// `scope`, i.e. '_$component.20deaad4-e86b-4ee2-97eb-92358d20db0d'
//
// `createComponent` function is not exported, used only in `partialFn`
// function.
function createComponent(view, model, Component, scope, ctx, macroCtx) {
  ...
  dom.nextUpdate(function() {
    ...
    for (var name in ctx.$elements) {
      if (!componentDom.element(name)) return;
      break;
    }

    if (component.create) component.create(scoped, componentDom);
...
// Not exported, called only from `sectionFn` and `pushVar` functions.
...
// Not exported, called only from `blockFn`.
...
// Not exported, called only from `parse` function.
...
// Used by `parseStart`, `parseMatch` and `parseEnd` functions.
...
// Used by `parseText` to parse `match` returned from `extractPlaceholder`.
...
// Parse `template` string using `parseText` function (defined below) if
// `isString` evaluates to `true`, of `parseHtml` (alias to `parse` from
// `html-util` package) otherwise.
//
// Used from `make` and `parseAttr` functions
/**
 * Parse `template`
 *
 ...
 * @param  {Boolean} isString   [description]
 * @param  {[type]}  onBind     [description]
 * @param  {[type]}  macroAttrs [description]
 * @return {[type]}             [description]
 */
function parse(view, viewName, template, isString, onBind) {
  ...
  // `parseText` function is defined inside a `parse` function because it should
  // be bound to `minifyContent` variable.

  /**
   * @param  {String}  text      [description]
   * @param  {Boolean} isRawText [description]
