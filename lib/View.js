...
, _afterRender: function(ns, ctx) {
  afterRender(this._appExports, ns, ctx);
}
...
function afterRender(app, ns, ctx) {
  app.emit('render', ctx);
...
function render(model, ns, ctx, silent) {
  ...
  if (silent) return dom._emitUpdate();
...
function createComponent(view, model, ns, name, scope, ctx, macroCtx, macroAttrs) {
  ...
  var componentDom = dom.componentDom()
    , cancelled;

  componentDom.element = function(name) {
