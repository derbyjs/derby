...
function Component(model, init, create) {
  this.model = model;
  this.init = init;
  this.create = create;
}
...
function createComponent(view, model, ns, name, scope, ctx, macroCtx, boundMacro) {
  ...
  var init = script.init
    ...
    , prefix = scope + '.'
    , component = new Component(scoped, init, create)
    , parentFnCtx = boundMacro.$fnCtx || ctx.$fnCtx
    , parent = true
    , fnCtx, i, key, path, value;

  ctx.$fnCtx = boundMacro.$fnCtx = parentFnCtx.concat(component);
  ...
  // TODO: This should probably emit on the app as well. Need to make
  // apps into event emitters to support this. Perhaps this should
  // replace the trigger method on compoment scoped models below
  for (i = parentFnCtx.length; i-- > 1;) {
    fnCtx = parentFnCtx[i];
    if (parent) {
      parent = false;
