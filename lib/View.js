...
function bindPathEvent(events, bindName, getName, partial, params) {
  events.push(function(ctx, modelEvents, dom, pathMap, view, blockPaths, triggerId) {
    ...
    var listener = modelListener(params, triggerId, blockPaths, pathId, partial, ctx);
    if (bindName !== getName) {
...
function bindEachPathEvent(events, name, getName, partial, params) {
  ...
  for (var i = segments.length; i; i--) {
    ...
    bindPathEvent(events, bindName, getName, partial, params);
  }
}
function bindEvents(events, name, partial, params) {
  if (~name.indexOf('(')) {
    ...
    for (var i = args.length; i--;) {
      bindPathEvent(events, args[i] + '*', name, partial, params);
    }
    return;
  }
