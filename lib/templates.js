...
var blockRegExp = /^\s*(if|unless|else if|each|with)\s+([\s\S]+?)(?:\s+as\s+(\S+)\s*)?$/;
var emptyBlockRegExp = /^\s*(else|unbound|bound)\s*$/;
var valueRegExp = /^\s*(?:(unbound|bound|unescaped)\s+)?([\s\S]*)/;
...
function createExpression(source) {
  ...
  // a path or alias.
  var match = blockRegExp.exec(source) || emptyBlockRegExp.test(source);
  var path;
  if (match) {
