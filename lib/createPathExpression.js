...
function reduceArrayExpression(node) {
  ...
  return (isLiteral) ?
    ...
    new expressions.FnExpression('[]', args);
}
...
function reduceSequenceExpression(node) {
  var args = node.expressions.map(reduce);
  var arg;
  for (var i = 0, len = args.length; i < len; i++) {
    var arg = args[i];
    if (arg instanceof expressions.LiteralExpression) continue;
    // Create a function expression if any expression isn't literal
    return new expressions.FnExpression(',', args);
  }
  // If all expressions are literal, return the last literal expression
  return arg;
}
