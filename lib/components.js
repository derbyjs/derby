  ...
  if (attributes) {
    for (var key in attributes) {
      ...
      if (attribute instanceof templates.ParentWrapper) {
        ...
        var value = renderAttribute(attribute, context);
        model.set(key, value);
      } else {
...
function renderAttribute(attribute, context) {
  if (typeof attribute !== 'object') return attribute;
  if (Array.isArray(attribute)) {
    var out = [];
    for (var i = 0, len = attribute.length; i < len; i++) {
      var value = renderAttributeProperties(attribute[i], context);
      out.push(value);
    }
    return out;
  }
  return renderAttributeProperties(attribute, context);
}
function renderAttributeProperties(attribute, context) {
  var out = {};
  for (var key in attribute) {
    var value = attribute[key];
    out[key] = (value instanceof templates.Template) ?
      value.template.get(context) : value;
  }
  return out;
}
