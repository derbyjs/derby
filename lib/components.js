...
Component.prototype.getAttribute = function(key) {
  ...
  var value = attributes && attributes[key];
  return value && expressions.renderValue(value, this.context);
};

function initComponent(context, component, parent, model, noHooks, id, scope) {
  ...
  if (!noHooks) emitHooks(context, component);
...
ComponentFactory.prototype.init = function(context, noHooks) {
  var component = new this.constructor();

  var parent = context.controller;
  var id = context.id();
  var scope = ['$components', id];
  var model = parent.model.root.scope(scope.join('.'));
  model.set('id', id);
  ...
  return initComponent(context, component, parent, model, noHooks, id, scope);
};
...
SingletonComponentFactory.prototype.init = function(context, noHooks) {
  var componentContext;
  if (this.component) {
    componentContext = context.componentChild(this.component);
    if (!noHooks) emitHooks(context, this.component);
  } else {
    this.component = new this.constructor();
    var parent = context.controller.page;
    var model = parent.model;
    componentContext = initComponent(context, this.component, parent, model, noHooks);
  }
  return componentContext;
};
