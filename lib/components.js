...
Component.prototype.destroy = function() {
  this.emit('destroy');
  // TODO: This can be removed once model.destroy() does this internally
  this.model.removeContextListeners();
  this.model.data.$controller = null;
  this.model.destroy();
  var bindings = this._bindings;
  if (bindings) {
    this._bindings = null;
    for (var i = bindings.length; i--;) {
      this.context.removeBinding(bindings[i]);
    }
  }
...
function setModelAttributes(component) {
  var attributes = component.context.parent.attributes;
  if (!attributes) return;
  // Set attribute values on component model
  for (var key in attributes) {
    var value = attributes[key];
    setModelAttribute(component, key, value);
  }
}

function setModelAttribute(component, key, value) {
