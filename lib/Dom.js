...
function makeSVGFragment(element, fragment, svgelement) {
...
function appendSVG(element, fragment, svgelement) {
  var frag = makeSVGFragment(element, fragment, svgelement);
...
function insertBeforeSVG(element, fragment, svgelement) {
  var frag = makeSVGFragment(element, fragment, svgelement);
  var svgchildren = frag.childNodes[0].childNodes;
  var parentNode, refElement;
  parentNode = element.parentNode;
  refElement = element;
  for(var i = 0, l = svgchildren.length; i < l; i++) {
    parentNode.insertBefore(svgchildren[0], refElement);
  }
}
...
function setHtml(obj, ignore, value, escape) {
  ...
  //handle case where obj is an SVG element
  var isSVG = obj.ownerSVGElement || obj.tagName === "svg";
  if(isSVG) {
