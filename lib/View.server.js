...
View.prototype._loadBundle = function(callback) {
  var bundle, templatesScript, bundleAndTemplates;
  if (isProduction) {
    if (bundleAndTemplates) return callback(null, bundleAndTemplates);
  } else if (bundle) {
    var view = this;
    var fileInfo = files.parseName(this._appFilename);
    loadTemplates(function(err) {
      if (err) return callback(err);
      callback(null, bundleAndTemplates);
    });
    return;
  }
