...
View.prototype._render = function(res, model, ns, ctx, isStatic, errors, css) {
  ...
  if (isStatic) {
    view._renderScripts(res, ns, ctx, isStatic, tail);
    return;
  }
...
View.prototype._renderScripts = function(res, ns, ctx, isStatic, tail, bundle) {
  ...
  if (!isStatic) {
    if (res.req.originalUrl !== res.req.url) {
      var mount = res.req.originalUrl.replace(res.req.url.slice(1), '');
      bundle.mount = mount;
    }
    scripts += "<script defer async onload='" +
      'require("derby").init(' +
      stringifyData(bundle) + ',' +
      stringifyData(flatten(ctx)) + ')' +
      "' src='" + (mount || '') + this._scriptPath + "'></script>";
  }
  res.write(scripts);
