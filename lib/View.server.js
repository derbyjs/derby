...
View.prototype.inline = function(fn) {
  var script = "(" + fn + ")()";
  if (racer.get('minify')) script = minifyJs(script);
  this._inline += script + ';';
};
...
View.prototype.render = function(res) {
  ...
  this._load(isStatic, function(err, errors, css, jsFile, appHash) {
    if (err) throw err;
    view._init(model);
    view._render(res, model, ns, ctx, isStatic, errors, css, jsFile, appHash);
  });
};
...
View.prototype._render = function(res, model, ns, ctx, isStatic, errors, css, jsFile, appHash) {
  errors || (errors = {});
  ...
  if (isStatic) {
    view._renderScripts(res, ns, ctx, isStatic, jsFile, appHash, tail);
    return;
  }
  model.del('_$components');
  model.bundle(function(bundle) {
    var renderHash = md5(header + body + footer);
    view._renderScripts(res, ns, ctx, isStatic, jsFile, appHash, tail, bundle, renderHash);
  });
};

View.prototype._renderScripts = function(res, ns, ctx, isStatic, jsFile, appHash, tail, bundle, renderHash) {
...
View.prototype._load = function(isStatic, callback) {
  ...
  load(this, isStatic, function(err, errors, css, jsFile, appHash) {
...
function load(view, isStatic, callback) {
  ...
  loadStyles(view, root, clientName, function(err, css) {
    if (err) errors['CSS'] = refresh.cssError(err);

    loadTemplates(view, root, clientName, function(err, templatesScript) {
      ...
      if (view._js) return writeJS(root, errors, css, view._js, templatesScript, callback);

      loadScripts(view, appFilename, function(err, js) {
        if (err) return callback(err);
        view._js = js;
        writeJS(root, errors, css, js, templatesScript, callback);
      });
    });
  });
}

function writeJS(root, errors, css, js, templatesScript, callback) {
  files.writeJs(root, js + '\n;' + templatesScript, function(err, jsFile, appHash) {
    if (err) return callback(err);
    callback(err, errors, css, jsFile, appHash);
  });
}

function loadStyles(view, root, clientName, callback) {
  view._loadStyles(root, clientName, function(err, value) {
    var css = '<style id=$_css>' + (value || '') + '</style>';
    callback(err, css);
  });
}

function loadTemplates(view, root, clientName, callback) {
  view._loadTemplates(root, clientName, function(err, templates, instances, libraryData) {
    ...
    if (racer.get('minify')) templatesScript = minifyJs(templatesScript);
    callback(err, templatesScript);
  });
}

function loadScripts(view, appFilename, callback) {
