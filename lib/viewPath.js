...
function replaceSquareBrackets(name) {
  var i = name.indexOf('[');
  if (i === -1) return name;
  ...
  var out = 'lookup(' + replaceSquareBrackets(inside) + ',' + before + ')';

  while (after) {
    i = after.indexOf('[');
    if (i === -1) return 'lookup("' + after.slice(1) + '",' + out + ')';
    ...
    if (before) out = 'lookup("' + before.slice(1) + '",' + out + ')';
    out = 'lookup(' + replaceSquareBrackets(inside) + ',' + out + ')';
  }
  return out;
}
