...
var events = exports.events = new EventDispatcher(
  function(listener, value, options) {
    ...
    if (_.isArray(listener)) {
      ...
      return dom.update(listener[0], listener[1], listener[2], listener[3], value, options);
...
var setters = {
  set: function(path, value) {
    var out = lookup(path);
    out.val = out.obj[out.prop] = value;
    return out;
  },
  push: function(path, value) {
    var out = lookup(path),
        arr = out.obj[out.prop];
    arr.push(value);
    out.val = arr;
    return out;
  }
...
_.forEach(setters, function(name, func) {
  exports[name] = function(path, value) {
    var out = func(path, value),
        path = out.path;
    events.trigger(path, out.val, name);
    if (_.publicModel(path)) send(name, [path, value]);
  };
