...
function onBind(path, listener) {
  function onRef(ref, key, refPath) {
...
var setters = {
  ...
  push: function(path, value) {
    var out = lookup(path),
        obj = out.obj;
    if (!obj.push) throw new Error('Invalid method "push" called on: ' + path);
    obj.push(value);
...
function lookup(path, options) {
  var obj = world,
      ...
      props, i, prop, len, ref, refObj, key, keyObj, funcName, func, inputs, next;
  if (path) {
    props = path.split('.');
    path = '';
    len = props.length;
    for (i = 0; prop = props[i++];) {
      ...
      if (ref = next.$r) {
        ...
        if (onRef) onRef(ref, key, path + '.' + props.slice(i).join('.'));
      
      } else {
        // Check for model functions
        if (funcName = next.$f) {
          inputs = funcInputs[funcName];
          inputs = (inputs.map) ? inputs.map(get) : [];
          func = funcs[funcName];
          if (func) next = func.apply(null, inputs);
          if (onFunc) onFunc(funcName, inputs);
        }
