...
function onTrigger(listener, value, options) {
  ...
  if (_.isArray(listener)) {
    ...
    return dom.update(id, method, property, viewFunc, value);
      ...
      (oldPathName = listener.$o) &&
      (pathName = listener.$p) &&
      (listenerObj = listener.$l)
    ) {
...
function onBind(pathName, listener) {
  var obj = world,
      path, i, prop, ref, key, eventPath, modelFunc, inputs;
  path = pathName.split('.');
  for (i = 0; prop = path[i++];) {
    obj = obj[prop];
    ...
    if (ref = obj.$r) {
      if (key = obj.$k) {
        eventPath = [ref, get(key)];
        // Register an event to update the other event handler when the
        // reference key changes
        events.bind(key, {$o: eventPath, $p: pathName, $l: listener});
      } else {
        eventPath = [ref];
      }
      eventPath = eventPath.concat(path.slice(i)).join('.');
