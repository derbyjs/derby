...
PathExpression.prototype.resolve = function(context) {
  if (this.relative) {
    return context.resolve().concat(this.segments);

  } else if (this.alias) {
    var alias = context.resolveAlias(this.alias);
    return alias.resolve().concat(this.segments);
...
// Get the segment list from the context
Context.prototype.resolve = function() {
  var segments = this.expression ? this.expression.resolve(this) : [];
  if (this.item != null) {
