...
var lookup = require('racer').util.lookup;
...
FnExpression.prototype.get = function(context) {
  ...
  var getFn = fn && fn.get;
  if (!getFn) {
    throw new Error('Function not found for: ' + this.name);
  }
  if (!this.args) return getFn.call(context.meta.page);
  var inputs = [];
  for (var i = 0, len = this.args.length; i < len; i++) {
    inputs.push(this.args[i].get(context));
  }
  var value = getFn.apply(context.meta.page, inputs);
...
function Context(meta, model, parent, expression, unbound, item, view, attributes) {
  this.meta = meta;
  this.model = model;
  this.parent = parent;
  ...
  // Only defined via Context::eachChild
  this.item = item;
  // Only defined via Context::viewChild
  this.view = view;
...
Context.prototype.child = function(expression) {
  ...
  var unbound = (blockType === 'unbound') ? true :
    ...
    this.unbound;
  return new Context(this.meta, this.model, this, expression, unbound);
};

// Make a context for an item in an each block
Context.prototype.eachChild = function(index) {
  return new Context(this.meta, this.model, this, this.expression, this.unbound, index);
};

Context.prototype.viewChild = function(view, attributes) {
  return new Context(this.meta, this.model, this, null, this.unbound, null, view, attributes);
};
