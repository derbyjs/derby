...
PathExpression.prototype.get = function(context) {
  if (this.relative) {
    var value = (this.meta && this.meta.blockType) ? context.parent.get() : context.get();
    value = lookup(this.segments, value);
    return getPatch(this, context, value);

  } else if (this.alias) {
    var value = context.resolveAlias(this.alias).get();
    value = lookup(this.segments, value);
    return getPatch(this, context, value);
...
BracketsExpression.prototype.resolve = function(context) {
  ...
  var segments = (this.afterSegments) ?
    segments.concat(insideSegments, this.afterSegments) :
    segments.concat(insideSegments);
...
function Context(meta, model, parent, expression, item) {
  this.meta = meta;
  this.model = model;
  this.parent = parent;
  this.expression = expression;
  this.item = item;
  // Set or inherit the binding mode. Default to bound at the root context
  this.bound = (expression && expression.meta.bindType) ? expression.meta.bindType === 'bound' :
    (parent) ? parent.bindType : true;
...
Context.prototype.eachChild = function(index) {
  return new Context(this.meta, this.model, this, this.expression, index);
};
...
function getPatch(expression, context, value) {
  return (expression === context.expression && context.item != null) ?
