...
function App(derby, name, filename, options) {
  ...
  this.scriptHash = `__DERBY_SCRIPT_HASH__`;
  this.bundledAt = `__DERBY_BUNDLED_AT__`;
...
App.prototype._init = function() {
  ...
  // depends on webpack dynamic modules being written to correct path/key
  var serializedViews = util.isServer
    ? require('./_views')
    : require(`./${this.name}__views`);
...
App.prototype._finishInit = function() {
  var state = this._getAppStateScript() ? this._getAppStateScript().innerHTML : this._initialState();
  var data = App._parseInitialData(state);
  ...
  this.emit('load', page);
  this._persistInitialState(state);
};
