...
function documentChange(e) {
  ...
  if (tagName === 'input') {
    ...
    setOptionBindings(target, target.value);
  }
}
...
function setOptionBindings(parent, value) {
  for (var node = parent.firstChild; node; node = node.nextSibling) {
    if (node.tagName && node.tagName.toLowerCase() === 'option') {
      var binding = node.$bindAttributes && node.$bindAttributes.selected;
      if (binding) binding.template.expression.set(binding.context, node.value === value);
    } else if (node.hasChildNodes()) {
      setOptionBindings(node, value);
    }
  }
}
